...
export function NotificationProvider({ children }) {
  ...
  /**
   * Resolves the async Firebase Messaging instance.
   */
  useEffect(() => {
    let active = true;
    messagingPromise.then((instance) => {
      if (active) setMessaging(instance);
    });
    return () => {
      active = false;
    };
  ...
  const deleteNotification = useCallback(async (notificationId) => {
    try {
      ...
      console.error('[LETCON] Error deleting notification:', error);
      throw error;
    }
  }, []);
  ...
  const deleteAllNotifications = useCallback(async () => {
    ...
    try {
      ...
      notifications.forEach((n) => {
        batch.delete(doc(db, COLLECTIONS.NOTIFICATIONS, n.id));
      });
      await batch.commit();
    } catch (error) {
      console.error('[LETCON] Error deleting all notifications:', error);
      throw error;
    }
  }, [user, notifications]);
