...
export function useRoleGuard(allowedRoles, {
  ...
  useEffect(() => {
    ...
    // Superadmin has full access regardless of allowedRoles.
    const isAllowed = userRole === ROLES.SUPER_ADMIN || normalizedAllowed.includes(userRole);
  ...
  const isAllowed = Boolean(
    user && userData &&
    (userRole === ROLES.SUPER_ADMIN || normalizedAllowed.includes(userRole))
  );
