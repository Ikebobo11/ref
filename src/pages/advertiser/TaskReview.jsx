...
export default function TaskReview() {
  ...
  const autoApprovalHours = settings?.autoApprovalHours ?? 24;
  ...
  const { data: submissions, loading } = useFirestoreQuery(COLLECTIONS.TASK_SUBMISSIONS, {
    filters: [
      { field: 'status', operator: '==', value: TASK_STATUS.SUBMITTED },
      { field: 'advertiserId', operator: '==', value: user?.uid || '' },
    ],
  ...
  return (
    <div className="dashboard-page">
      <PageHeader
        title="Task Review"
        subtitle={`Approve or reject submissions. Auto-approved after ${autoApprovalHours} hours if no action.`}
      ...
      <div className="auto-approval-note">
        <FaClock />
        <span>
          If you do not review a submission within {autoApprovalHours} hours, it will be
