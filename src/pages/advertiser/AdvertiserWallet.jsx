...
import { createFundWalletSchema } from '../../utils/validators';
...
export default function AdvertiserWallet() {
  ...
  const {
    ...
    resolver: zodResolver(createFundWalletSchema(settings)),
  });
    ...
    try {
      ...
      const handler = createCheckoutHandler({
        ...
        onSuccess: async (response) => {
          try {
            // Verify the payment with Paystack and credit the wallet
            await verifyAndCreditWallet(response.reference || reference, user.uid, data.amount);
            toast.success('Wallet funded successfully!');
            refreshWallet();
          } catch (error) {
            console.error('[LETCON] Wallet funding verification error:', error);
