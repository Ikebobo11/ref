...
export default function CreateCampaign() {
  ...
  // Build option lists dynamically from settings (fall back to constants)
  const platformList = settings?.platformList?.length ? settings.platformList : PLATFORM_LIST;
  const tierList = settings?.tierList?.length ? settings.tierList : TIER_LIST;
  const countryList = settings?.countries?.length ? settings.countries : COUNTRIES;
  const PLATFORM_OPTIONS = platformList.map((p) => ({ value: p, label: p }));
  const TIER_OPTIONS = tierList.map((t) => ({ value: t, label: `${t} followers` }));
  const COUNTRY_OPTIONS = countryList.map((c) => ({ value: c, label: c }));
  ...
  const {
    ...
    resolver: zodResolver(createTaskSchema(settings)),
    defaultValues: {
      targetGender: 'any',
    },
  });
  ...
  const paymentPerInfluencer = getTierPayment(followerTier, platform, settings);
  const [totalPrice, setTotalPrice] = useState(0);

  // Revenue split from settings (falls back to 30% platform / 70% earner)
  const platformRevenuePercent = settings?.platformRevenuePercent ?? 30;
  const earnerRevenuePercent = 100 - platformRevenuePercent;

  // Determine the posting fee: gender-specific campaigns use the higher fee
  const genderPostingFee = settings?.genderPostingFee ?? FEES.GENDER_POSTING_FEE;
  const standardPostingFee = settings?.taskPostingFee ?? FEES.TASK_POSTING_FEE;
  const postingFee = targetGender && targetGender !== 'any' ? genderPostingFee : standardPostingFee;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const totalRequired = totalPrice + postingFee;
          ...
          <CardBody>
            <form onSubmit={handleSubmit(onSubmit)} className="form-stack">
              ...
              <Select
                ...
                error={errors.targetGender?.message}
              ...
              <div className="payment-preview">
                ...
                <span>Platform keeps {platformRevenuePercent}%: {formatNaira(Math.round(totalPrice * platformRevenuePercent / 100))}</span>
                <span>Earner receives {earnerRevenuePercent}%: {formatNaira(Math.round(totalPrice * earnerRevenuePercent / 100))}</span>
                <span>Posting fee: {formatNaira(postingFee)}</span>
                <span>Total to deduct from wallet: <strong>{formatNaira(totalPrice + postingFee)}</strong></span>
              </div>
              ...
              <div className="posting-fee-note">
                <FaCreditCard />
                <div>
                  <strong>Task Posting Fee: {CURRENCY_SYMBOL}{postingFee.toLocaleString()}</strong>
                  <p>
                    {targetGender && targetGender !== 'any'
                      ? `Gender-specific campaign (${targetGender}) — higher posting fee applies. Non-refundable. Deducted from your wallet balance.`
                      : 'Non-refundable. Deducted from your wallet balance.'}
                  </p>
                </div>
              </div>
