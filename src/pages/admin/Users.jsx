/**
 * LETCON - Users Page
 * Admin manages users: view, suspend, ban accounts, and send notifications.
 */
...
export default function Users() {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const actorRole = userData?.role || 'admin';
      if (action === 'suspend') {
        await suspendUser(user.id, userData.uid, actorRole, reason);
        toast.success(`Suspended ${user.fullName}`);
      } else if (action === 'unsuspend') {
        await unsuspendUser(user.id, userData.uid, actorRole);
        toast.success(`Unsuspended ${user.fullName}`);
      } else if (action === 'ban') {
        await banUser(user.id, userData.uid, actorRole, reason);
  ...
  /**
   * Opens the message modal for a user.
  ...
  const handleSendMessage = async () => {
    if (!messageModal || !messageTitle.trim() || !messageBody.trim()) {
    ...
    try {
      await createNotification({
        ...
        data: {
          sentBy: userData?.uid,
          sentByRole: userData?.role || 'admin',
        },
      });
      toast.success(`Message sent to ${messageModal.fullName}`);
  ...
  return (
    <div className="dashboard-page">
      ...
      {/* Action Modal (Suspend/Ban/Unsuspend) */}
      <Modal
        ...
        {messageModal && (
          <div className="message-form">
            <div className="action-user-info">
              <FaEnvelope />
              <div>
                <strong>{messageModal.fullName}</strong>
            ...
            <p className="form-helper">
              This message will be sent as a notification to the user's notification section.
            </p>
            <Button
              variant="primary"
              fullWidth
