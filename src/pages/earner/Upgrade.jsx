...
import { createUpgradeSchema } from '../../utils/validators';
...
export default function Upgrade() {
  ...
  const tierMinFollowers = settings?.tierMinFollowers ?? TIER_MIN_FOLLOWERS;
  const currentTier = userData?.tier;
  const nextTier = getNextTier(currentTier, settings);
  ...
  const {
    ...
    resolver: zodResolver(createUpgradeSchema(settings)),
  });
          ...
          <CardBody>
            <form onSubmit={handleSubmit(onSubmit)} className="form-stack">
              <div className="upgrade-info-banner">
                ...
                <span>Next Tier: {nextTier} (min {tierMinFollowers[nextTier].toLocaleString()} followers)</span>
              </div>

              <Input
                label="New Follower Count"
                type="number"
                placeholder={`Minimum ${tierMinFollowers[nextTier].toLocaleString()} for ${nextTier}`}
                icon={<FaUsers />}
          ...
          <CardBody>
            <div className="info-list">
              <div className="info-item">
                ...
                <p>Your old tier is removed and your new tier is assigned. You will immediately begin seeing only tasks in the new tier.</p>
              </div>
              <div className="info-item">
                <strong>Platform stays the same</strong>
                <p>Your verified platform ({userData?.verifiedPlatform}) remains unchanged. This is a follower-count tier upgrade only.</p>
              </div>
              <div className="info-item">
                <strong>Admin review</strong>
                <p>An admin will review your new screenshot and profile link before approving.</p>
              </div>
            </div>
          </CardBody>
        </Card>
