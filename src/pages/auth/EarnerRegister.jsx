...
import { createEarnerRegisterSchema } from '../../utils/validators';
...
export default function EarnerRegister() {
  ...
  // Build platform options dynamically from settings (fall back to constants)
  const platformList = settings?.platformList?.length ? settings.platformList : PLATFORM_LIST;
  const PLATFORM_OPTIONS = platformList.map((platform) => ({ value: platform, label: platform }));
  ...
  const {
    ...
    resolver: zodResolver(createEarnerRegisterSchema(settings)),
    defaultValues: {
      platform: '',
      platformName: '',
      username: '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Create verification request. If the verification fee is set to ₦0 (FREE promotion),
      // no payment is required — the request is auto-marked paid and registration completes
      // directly without loading Paystack.
      const reference = generateReference('VER');
      const isFreeRegistration = verificationFee <= 0;
      await setDocument(COLLECTIONS.VERIFICATION_REQUESTS, reference, {
        uid: createdUser.uid,
        email: data.email,
        fullName: data.fullName,
      ...
      if (isFreeRegistration) {
        toast.success('Account created successfully! Registration is FREE — your account is pending admin verification.');
      ...
      navigate('/verification-payment', { state: { reference, email: data.email } });
    } catch (error) {
    ...
    <div className="auth-form auth-form-wide">
      ...
      {verificationFee > 0 ? (
        <div className="verification-fee-notice">
          <FaCreditCard />
          <div>
            <strong>Verification Fee: {CURRENCY_SYMBOL}{verificationFee.toLocaleString()}</strong>
            <p>Non-refundable. Paid after registration to verify your account.</p>
          </div>
        </div>
      ) : (
        <div className="verification-fee-notice free">
          <FaCircleCheck />
          <div>
            <strong>Registration is currently FREE (₦0)</strong>
            <p>No verification fee required. Your account will be reviewed by an admin after registration.</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="auth-form-fields">
        ...
        <Button type="submit" fullWidth loading={loading}>
          <FaRightToBracket /> {verificationFee > 0 ? 'Create Account & Continue to Payment' : 'Create Account'}
        </Button>
      </form>
