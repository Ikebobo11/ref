...
export async function fundWallet({ uid, amount, reference }) {
  // Enforce super-admin configured funding limits (fall back to defaults)
  const minFunding = getSettingSync('minWalletFunding', 100);
  const maxFunding = getSettingSync('maxWalletFunding', 10000000);
  if (amount < minFunding) {
    throw new Error(`Minimum wallet funding is ₦${minFunding.toLocaleString()}`);
  }
  if (amount > maxFunding) {
    throw new Error(`Maximum wallet funding is ₦${maxFunding.toLocaleString()}`);
  }
...
/**
 * Deducts a campaign budget from an advertiser's wallet.
 * @param {Object} options - Deduction options.
 * @param {string} options.uid - The advertiser's user ID.
 * @param {number} options.amount - The amount to deduct.
 * @param {string} options.taskId - The task ID.
 * @returns {Promise<Object>} The updated wallet.
 */
export async function deductCampaignBudget({ uid, amount, taskId }) {
  return executeTransaction(async (transaction) => {
    const walletRef = doc(db, COLLECTIONS.WALLETS, uid);
    const walletSnapshot = await transaction.get(walletRef);
    ...
    if (currentBalance < amount) {
      throw new Error('Insufficient wallet balance to cover campaign budget');
    }
    ...
    transaction.update(walletRef, {
      balance: newBalance,
      totalSpent: (walletSnapshot.data().totalSpent || 0) + amount,
      updatedAt: new Date(),
    });

    // Record transaction
    const reference = generateReference('BUDGET');
    const txnRef = doc(db, COLLECTIONS.TRANSACTIONS, reference);
    transaction.set(txnRef, {
      uid,
      ...
      reference,
      taskId,
      description: 'Campaign budget escrow',
      createdAt: new Date(),
    });
...
/**
 * Deducts the task posting fee from an advertiser's wallet.
 * @param {Object} options - Deduction options.
 * @param {string} options.uid - The advertiser's user ID.
 * @param {number} options.amount - The fee amount.
 * @param {string} [options.taskId] - The task ID.
 * @returns {Promise<Object>} The updated wallet.
 */
export async function deductPostingFee({ uid, amount, taskId }) {
  return executeTransaction(async (transaction) => {
    const walletRef = doc(db, COLLECTIONS.WALLETS, uid);
    const walletSnapshot = await transaction.get(walletRef);
    ...
    const currentBalance = walletSnapshot.data().balance || 0;
    if (currentBalance < amount) {
      throw new Error('Insufficient wallet balance to cover the posting fee');
    }
    ...
    transaction.set(txnRef, {
      uid,
      type: TRANSACTION_TYPES.TASK_POSTING_FEE,
      amount,
      status: TRANSACTION_STATUS.SUCCESS,
      reference,
      taskId,
...
export async function createWithdrawal({ uid, amount, bankName, accountNumber, accountName }) {
  // Enforce super-admin configured withdrawal limits (fall back to defaults)
  const minWithdrawal = getSettingSync('minWithdrawal', 1000);
  const maxWithdrawal = getSettingSync('maxWithdrawal', 10000000);
...
/**
 * Checks if a withdrawal reference is already used (prevents duplicates).
 * @param {string} reference - The withdrawal reference.
 * @returns {Promise<boolean>} True if the reference exists.
 */
export async function withdrawalExists(reference) {
  const existing = await queryDocuments(COLLECTIONS.WITHDRAWALS, {
