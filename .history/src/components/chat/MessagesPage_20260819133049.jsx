...
export default function MessagesPage({ subtitle = 'Chat with LETCON support' }) {
  ...
  // Uses setDoc WITHOUT merge so Firestore treats it as a create operation.
  // If the document already exists, the operation is a no-op (caught silently).
  const ensureConversation = useCallback(async () => {
    ...
    try {
      // Try to read first — if it exists, we're done
      ...
      // Read failed (permission or not found) — proceed to create
    }

    // Create the conversation document (setDoc without merge = create)
    try {
      ...
      // If the document was already created by another operation (race condition),
      // the setDoc will fail because it already exists. That's fine — just use it.
