...
/**
 * Invites a new admin.
 * Finds the user by email and updates their role to 'admin' so they
 * can access the admin dashboard immediately.
...
export async function inviteAdmin({ email, fullName, superAdminId }) {
  ...
  if (users.length > 0) {
    userId = users[0].id;
    // Update the user's role to admin so they can access admin dashboard
  ...
  // Fire-and-forget audit log (don't block the user's action)
  logAdminInvite(superAdminId, ROLES.SUPER_ADMIN, invite.id, { email }).catch(console.error);

  return { ...invite, uid: userId };
}
...
export async function removeAdmin(adminId, superAdminId) {
  ...
  // Prevent removing a super admin account
  ...
  // Revert the user's role back to advertiser if they have a uid
  if (admin.uid) {
    await updateDocument(COLLECTIONS.USERS, admin.uid, {
      role: ROLES.ADVERTISER,
    });
  }

  // Fire-and-forget audit log (don't block the user's action)
  logAdminRemove(superAdminId, ROLES.SUPER_ADMIN, adminId, { email: admin.email }).catch(console.error);
}
 ...
 * @param {Object} options - Query options.
 * @returns {Promise<Array<Object>>} Array of flagged submissions.
 */
export async function getFlaggedSubmissions({ limitCount = 50 } = {}) {
  return queryDocuments('taskSubmissions', {
...
/**
 * Gets tasks pending admin review.
 * @param {Object} options - Query options.
 * @returns {Promise<Array<Object>>} Array of pending review tasks.
 */
export async function getPendingAdminReviewTasks({ limitCount = 50 } = {}) {
  return queryDocuments('taskSubmissions', {
