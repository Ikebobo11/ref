...
/**
 * Dynamically loads the Paystack inline script when needed.
 * This avoids loading the script on every page and prevents
 * browser tracking prevention from blocking it.
...
export async function recordVerificationFeePayment({ uid, email, reference }) {
  ...
  const transaction = await setDocument(COLLECTIONS.TRANSACTIONS, reference, {
    ...
    amount: getSettingSync('verificationFee', 1000),
    status: 'pending',
    reference,
...
export async function recordTaskPostingFee({ uid, email, reference, taskId }) {
  const transaction = await setDocument(COLLECTIONS.TRANSACTIONS, reference, {
    ...
    amount: getSettingSync('taskPostingFee', 1000),
    reference,
    taskId,
 ...
 * Dynamically loads the Paystack script when the handler is invoked,
 * so it's only loaded when a user actually makes a payment.
 ...
 * @returns {Function} Async handler function that opens the checkout.
 */
export function createCheckoutHandler({ email, amount, reference, onSuccess, onCancel, metadata }) {
  return async () => {
    ...
    // Dynamically load the Paystack script only when needed
    try {
      await loadPaystackScript();
    } catch (error) {
      console.error('[LETCON]', error);
      throw error;
    }
