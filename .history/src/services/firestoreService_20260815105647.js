 ...
 * @param {Function} callback - Callback with the documents array and optional error.
 * @returns {Function} Unsubscribe function.
 */
export function subscribeToQuery(collectionName, {
  filters = [],
  orderByFields = [],
  limitCount = 20,
} = {}, callback) {
  ...
  return onSnapshot(
    ...
    (error) => {
      console.error(`[LETCON] Error subscribing to ${collectionName}:`, error);
      callback([], error);
    }
  );
}
 ...
 * @param {Array<Object>} operations - Array of { type, collectionName, docId, data } operations.
 * @returns {Promise<void>}
 */
 ...
 * @param {Function} transactionFn - Function that receives a transaction object.
