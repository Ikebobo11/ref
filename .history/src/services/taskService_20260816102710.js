...
export async function approveTaskSubmission(submissionId, approvedBy, approvalType = 'advertiser') {
  ...
  const [submission, task, earnerWallet] = await Promise.all([
    ...
    (async () => {
      // If the task id is missing, short-circuit early.
      // We'll fetch it after submission is available.
      return null;
    })(),
    null,
  ]);
  ...
  // We had to fetch the submission first to know the task ID — get task + wallet in parallel now.
  const taskPromise = getDocument(COLLECTIONS.TASKS, submission.taskId);
  const walletPromise = getDocument(COLLECTIONS.WALLETS, submission.earnerId);

  const [resolvedTask, resolvedWallet] = await Promise.all([taskPromise, walletPromise]);
