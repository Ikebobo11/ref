...
export async function getOrCreateConversation({
  ...
  // Try to read existing conversation first (may fail if it doesn't exist
  // due to Firestore rules, so we catch and proceed to create).
  try {
    const existing = await getDocument('conversations', conversationId);
    if (existing) return existing;
  } catch {
    // Document doesn't exist or permission denied — proceed to create
  }
  ...
  // Create the conversation if it doesn't exist (merge: true preserves
  // existing data if another participant already created it).
...
export async function sendDirectMessage({
  ...
  try {
    ...
    // Document doesn't exist or permission denied
  }
  if (!conversation) {
