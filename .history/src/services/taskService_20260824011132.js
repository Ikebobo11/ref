...
export async function acceptTask(taskId, earner) {
  ...
  const acceptance = await addDocument('taskAcceptances', {
    ...
    acceptedAt: new Date(),
    autoApprovalDeadline: new Date(Date.now() + (getSettingSync('autoApprovalHours', 24) * 60 * 60 * 1000)),
  });
...
export async function submitTaskProof(taskId, submission, earner) {
  ...
  const submissionData = {
    ...
    autoApprovalDeadline: new Date(Date.now() + (getSettingSync('autoApprovalHours', 24) *
  };
