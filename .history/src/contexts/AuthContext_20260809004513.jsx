...
export function AuthProvider({ children }) {
  ...
  /**
   * Fetches the user's profile data from Firestore with auto
  const fetchUserData = useCallback(async (firebaseUser) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (detectedRole) {
          return await ensureUserDocuments(firebaseUser, detectedRole);
        }
        // No role anywhere - default to earner
        return await ensureUserDocuments(firebaseUser, ROLES.EARNER);
      }
