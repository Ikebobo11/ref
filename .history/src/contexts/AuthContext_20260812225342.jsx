...
export function AuthProvider({ children }) {
  ...
  // sessionStorage key for caching the user's role hint between sessions,
  // so login routing still works if Firestore rules/network briefly fail.
  const ROLE_HINT_KEY = 'letcon_role_hint';

  /**
   * Stores the user's role as a session hint for fallback routing.
   * @param {string} uid - The user's UID.
   * @param {string} role - The user's role.
   */
  const setRoleHint = useCallback((uid, role) => {
    try {
      sessionStorage.setItem(ROLE_HINT_KEY, JSON.stringify({ uid, role }));
    } catch {
  ...
  /**
   * Retrieves the stored role hint for a given UID, if any.
   * @param {string} uid - The user's UID.
   * @returns {string|null} The cached role or null.
   */
  const getRoleHint = useCallback((uid) => {
    try {
      const raw = sessionStorage.getItem(ROLE_HINT_KEY);
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      return parsed && parsed.uid === uid ? parsed.role : null;
    } catch {
      return null;
    }
  }, []);
  ...
  /**
   * Clears the stored role hint.
   */
  const clearRoleHint = useCallback(() => {
    try {
  ...
  const fetchUserData = useCallback(async (firebaseUser, forceRefresh = false) => {
    ...
    try {
      ...
      // Cache the result and store a role hint for resilient login routing
      userDataCache.current.set(uid, result);
      if (result?.role) {
        setRoleHint(uid, result.role);
      }
      return result;
    } catch (error) {
      ...
      // Last-resort fallback: use the role hint cached in sessionStorage so the
      // user can still be routed to their dashboard instead of being hard-blocked.
      const roleHint = getRoleHint(uid);
      if (roleHint) {
        const hintFallback = {
          id: uid,
          uid,
        ...
        userDataCache.current.set(uid, hintFallback);
        return hintFallback;
      }

      return null;
    }
  }, [detectRole, ensureUserDocuments, setRoleHint, getRoleHint]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        const data = await fetchUserData(firebaseUser, true);
        setUserData(data);
      } else {
        setUserData(null);
  ...
  const register = async ({ email, password, fullName, role, profile = {} }) => {
    try {
      ...
      // Store the role hint immediately so login routing works even if the
      // Firestore writes below are denied by rules (e.g., before deployment).
      setRoleHint(uid, role);
