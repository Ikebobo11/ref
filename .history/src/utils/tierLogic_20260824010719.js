...
function _getTiersMapOld(settings) {
 ...
 */
function _getTierMinFollowersMap(settings) {
  const s = _resolveSettings(settings);
  if (s?.tierMinFollowers) return s.tierMinFollowers;
...
function _getTierPaymentsMap(settings) {
  const s = _resolveSettings(settings);
  if (s?.tierPayments) return s.tierPayments;
...
function _getRevenueSplitMap(settings) {
  const s = _resolveSettings(settings);
  if (s?.platformRevenuePercent !== undefined) {
    return {
      PLATFORM_PERCENT: s.platformRevenuePercent,
      EARNER_PERCENT: 100 - s.platformRevenuePercent,
    };
  }
...
function _getPlatformListArr(settings) {
  const s = _resolveSettings(settings);
  if (s?.platformList?.length) return s.platformList;
...
export function getTierFromFollowers(followerCount, settings) {
  const s = _resolveSettings(settings);
  const minFollowers = _getTierMinFollowersMap(s);
  const tiers = _getTiersMap(s);
  const tierList = s?.tierList || Object.values(tiers);
...
export function getTierPayment(tier, platform, settings) {
  const payments = _getTierPaymentsMap(_resolveSettings(settings));
...
export function calculateRevenueSplit(taskAmount, settings) {
  const split = _getRevenueSplitMap(_resolveSettings(settings));
...
export function getTierMinFollowers(tier, settings) {
  const minFollowers = _getTierMinFollowersMap(_resolveSettings(settings));
...
export function isValidPlatform(platform, settings) {
  const platforms = _getPlatformListArr(_resolveSettings(settings));
