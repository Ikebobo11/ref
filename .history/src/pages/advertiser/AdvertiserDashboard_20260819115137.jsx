...
export default function AdvertiserDashboard() {
  ...
  const { data: tasks, loading: tasksLoading, error: tasksError } = useFirestoreQuery(COLLECTIONS.TASKS, {
    filters: [{ field: 'advertiserId', operator: '==', value: userData?.uid || '' }],
    orderByFields: [{ field: 'createdAt', direction: 'desc' }],
    limitCount: 5,
  });

  const { data: submissions, loading: submissionsLoading, error: submissionsError } = useFirestoreQuery(COLLECTIONS.TASK_SUBMISSIONS, {
    filters: [{ field: 'status', operator: '==', value: TASK_STATUS.SUBMITTED }],
    orderByFields: [{ field: 'submittedAt', direction: 'desc' }],
    limitCount: 5,
  });

  if (walletLoading || (tasksLoading && !tasksError) || (submissionsLoading && !submissionsError)) {
  ...
  return (
    <div className="dashboard-page">
      ...
      <div className="dashboard-grid">
        ...
        <Card className="dashboard-section">
          <CardHeader>
            ...
            <Link to="/advertiser/task-review" className="text-link">Review</Link>
          </CardHeader>
          <CardBody>
      ...
      <div className="quick-actions">
        <Link to="/advertiser/create-campaign" className="quick-action-btn">
          <FaCirclePlus /> Create Campaign
        </Link>
        <Link to="/advertiser/wallet" className="quick-action-btn">
          <FaWallet /> Fund Wallet
        </Link>
        <Link to="/advertiser/task-review" className="quick-action-btn">
          <FaClockRotateLeft /> Review Tasks
        </Link>
      </div>
    </div>
  );
