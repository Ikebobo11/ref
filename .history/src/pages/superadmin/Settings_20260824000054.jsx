  ...
  useEffect(() => {
    if (settings) {
      setForm({
        ...
        genderPostingFee: settings.genderPostingFee ?? 3000,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        genderPostingFee: form.genderPostingFee,
      ...
      <div className="settings-content">
        ...
        {activeTab === 'tiers' && (
          <>
            <Card>
              ...
              <CardBody>
                {form.tierList.length === 0 ? (
                  <p className="text-muted">No tiers defined. Add a tier to get started.</p>
                ) : (
                  <div className="settings-table-wrapper">
                    <table className="settings-table">
                      ...
                      <tbody>
                        {form.tierList.map((tier) => (
                          <tr key={tier}>
                            <td><strong>{tier}</strong></td>
                            <td>
                              <Input
                                type="number"
                                value={form.tierMinFollowers[tier] || 0}
                                onChange={(e) => updateTierMinFollowers(tier, e.target.value)}
                                style={{ width: '140px' }}
                              />
                            </td>
                            <td>
                              <Button variant="danger" size="sm" onClick={() => removeTier(tier)}>
                                <FaTrash />
                              </Button>
                            </td>
                          </tr>
            ...
            <Card style={{ marginTop: '1rem' }}>
              <CardHeader>
                <CardTitle>Tier Payment Matrix (₦)</CardTitle>
              </CardHeader>
              <CardBody>
                {form.tierList.length === 0 || form.platformList.length === 0 ? (
                  <p className="text-muted">Add tiers and platforms to configure payments.</p>
                ) : (
                  <div className="settings-table-wrapper">
                    <table className="settings-table">
                      <thead>
                      ...
                      <tbody>
                        {form.tierList.map((tier) => (
                          <tr key={tier}>
                            <td><strong>{tier}</strong></td>
                            {form.platformList.map((platform) => (
                              <td key={platform}>
                                <Input
                                  type="number"
                                  value={form.tierPayments[tier]?.[platform] ?? 0}
                                  onChange={(e) => updateTierPayment(tier, platform, e.target.value)}
                                  style={{ width: '100px' }}
                                />
                              </td>
                            ))}
        ...
        {activeTab === 'platforms' && (
          <Card>
            ...
            <CardBody>
              {form.platformList.length === 0 ? (
                <p className="text-muted">No platforms defined.</p>
              ) : (
                <div className="settings-list">
                  {form.platformList.map((platform) => (
                    <div key={platform} className="settings-list-item">
                      <span>{platform}</span>
                      <Button variant="danger" size="sm" onClick={() => removePlatform(platform)}>
                        <FaTrash />
                      </Button>
                    </div>
                  ))}
        ...
        {activeTab === 'pricing' && (
          <Card>
            ...
            <CardBody>
              <div className="form-stack">
                <Input
                  label={`Verification Fee (${form.currencySymbol})`}
                  type="number"
                  icon={<FaCreditCard />}
                  value={form.verificationFee}
                  onChange={(e) => updateField('verificationFee', parseInt(e.target.value) || 0)}
                />
                <Input
                  label={`Task Posting Fee (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.taskPostingFee}
                  onChange={(e) => updateField('taskPostingFee', parseInt(e.target.value) || 0)}
                />
                <Input
                  label={`Gender-Specific Posting Fee (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.genderPostingFee}
                  onChange={(e) => updateField('genderPostingFee', parseInt(e.target.value) || 0)}
                  hint="Applied when a campaign targets a specific gender (male or female)"
                />
                <Input
                  label="Platform Revenue Percent (%)"
                  type="number"
                  icon={<FaPercent />}
                  value={form.platformRevenuePercent}
                  onChange={(e) => updateField('platformRevenuePercent', parseInt(e.target.value) || 0)}
                  hint={`Earners receive ${100 - (form.platformRevenuePercent || 0)}%`}
                />
                <Input
                  label="Auto-Approval Hours"
                  type="number"
                  value={form.autoApprovalHours}
                  onChange={(e) => updateField('autoApprovalHours', parseInt(e.target.value) || 0)}
                  hint="Tasks auto-approve after this many hours if not reviewed"
                />
                <Input
                  label={`Min Withdrawal (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.minWithdrawal}
                  onChange={(e) => updateField('minWithdrawal', parseInt(e.target.value) || 0)}
                />
                <Input
                  label={`Max Withdrawal (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.maxWithdrawal}
                  onChange={(e) => updateField('maxWithdrawal', parseInt(e.target.value) || 0)}
                />
                <Input
                  label={`Min Wallet Funding (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.minWalletFunding}
                  onChange={(e) => updateField('minWalletFunding', parseInt(e.target.value) || 0)}
                />
                <Input
                  label={`Max Wallet Funding (${form.currencySymbol})`}
                  type="number"
                  icon={<FaWallet />}
                  value={form.maxWalletFunding}
                  onChange={(e) => updateField('maxWalletFunding', parseInt(e.target.value) || 0)}
                />
                <Input
                  label="Min Followers to Register"
                  type="number"
                  icon={<FaHashtag />}
                  value={form.minFollowersToRegister}
                  onChange={(e) => updateField('minFollowersToRegister', parseInt(e.target.value) || 0)}
                />
              </div>
            </CardBody>
          </Card>
        ...
        {activeTab === 'uploads' && (
          <>
              ...
              <CardBody>
                <div className="form-stack">
                  <Input
                    ...
                    onChange={(e) => updateField('maxImageSizeMB', parseInt(e.target.value) || 0)}
                  />
                  <Input
                    label="Max Video Size (MB)"
                    type="number"
                    value={form.maxVideoSizeMB}
                    onChange={(e) => updateField('maxVideoSizeMB', parseInt(e.target.value) || 0)}
                  />
                  <Input
                    label="Max Images Per Task"
                    type="number"
                    value={form.maxImagesPerTask}
                    onChange={(e) => updateField('maxImagesPerTask', parseInt(e.target.value) || 0)}
                  />
                  <Input
                    label="Max Videos Per Task"
                    type="number"
                    value={form.maxVideosPerTask}
                    onChange={(e) => updateField('maxVideosPerTask', parseInt(e.target.value) || 0)}
                  />
                </div>
              </CardBody>
            </Card>
            ...
            <Card style={{ marginTop: '1rem' }}>
              <CardHeader>
                <CardTitle>Allowed Image Types</CardTitle>
              ...
              <CardBody>
                <div className="settings-list">
                  {form.allowedImageTypes.map((mime) => (
                    <div key={mime} className="settings-list-item">
                      <code>{mime}</code>
                      <Button variant="danger" size="sm" onClick={() => removeImageType(mime)}>
                        <FaTrash />
                      </Button>
                    </div>
                  ))}
            ...
            <Card style={{ marginTop: '1rem' }}>
              <CardHeader>
                <CardTitle>Allowed Video Types</CardTitle>
                <Button variant="outline" onClick={addVideoType}>
                  <FaPlus /> Add Type
                </Button>
              </CardHeader>
              <CardBody>
                <div className="settings-list">
                  {form.allowedVideoTypes.map((mime) => (
                    <div key={mime} className="settings-list-item">
                      <code>{mime}</code>
                      <Button variant="danger" size="sm" onClick={() => removeVideoType(mime)}>
                        <FaTrash />
                      </Button>
                    </div>
                  ))}
            ...
            <CardBody>
              <div className="settings-list">
                {form.countries.map((country) => (
                  <div key={country} className="settings-list-item">
                    <span>{country}</span>
                    <Button variant="danger" size="sm" onClick={() => removeCountry(country)}>
                      <FaTrash />
                    </Button>
                  </div>
                ))}
        ...
        {activeTab === 'pagination' && (
          <Card>
            <CardHeader>
              <CardTitle>Pagination & Query Limits</CardTitle>
            </CardHeader>
            <CardBody>
              <div className="form-stack">
                <Input
                  ...
                  type="number"
                  value={form.pageSize}
                  onChange={(e) => updateField('pageSize', parseInt(e.target.value) || 0)}
                />
                <Input
                  label="Query Limit (Default)"
                  type="number"
                  value={form.queryLimitDefault}
                  onChange={(e) => updateField('queryLimitDefault', parseInt(e.target.value) || 0)}
                />
                <Input
                  label="Query Limit (Large)"
                  type="number"
                  value={form.queryLimitLarge}
                  onChange={(e) => updateField('queryLimitLarge', parseInt(e.target.value) || 0)}
                />
                <Input
                  label="Query Limit (Max)"
                  type="number"
                  value={form.queryLimitMax}
                  onChange={(e) => updateField('queryLimitMax', parseInt(e.target.value) || 0)}
                />
              </div>

              <div style={{ marginTop: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  <h4 style={{ margin: 0 }}>Page Size Options</h4>
                  <Button variant="outline" size="sm" onClick={addPageSizeOption}>
                    <FaPlus /> Add Option
                  </Button>
                </div>
                <div className="settings-list">
                  {form.pageSizeOptions.map((opt) => (
                    <div key={opt} className="settings-list-item">
                      <span>{opt}</span>
                      <Button variant="danger" size="sm" onClick={() => removePageSizeOption(opt)}>
                        <FaTrash />
                      </Button>
                    </div>
                  ))}
